...
export default function AddPatient() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.address.street.trim()) newErrors['address.street'] = 'Required';
    if (!formData.address.city.trim()) newErrors['address.city'] = 'Required';
    if (!formData.address.state.trim()) newErrors['address.state'] = 'Required';
    if (!formData.address.zipCode.trim()) newErrors['address.zipCode'] = 'Required';
    if (!formData.emergencyContact.name.trim()) newErrors['emergencyContact.name'] = 'Required';
    if (!formData.emergencyContact.relationship.trim()) newErrors['emergencyContact.relationship'] = 'Required';
    if (!formData.emergencyContact.phone.trim()) newErrors['emergencyContact.phone'] = 'Required';
    setErrors(newErrors);
      ...
      <form onSubmit={handleSubmit} className="space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
          ...
          <div className="md:col-span-8 bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm space-y-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              <FormField label="Date of Birth" type="date" value={formData.dateOfBirth} onChange={v => updateFormData('dateOfBirth', v)} required error={errors.dateOfBirth} />
              <SelectField label="Gender" value={formData.gender} onChange={v => updateFormData('gender', v)} options={[{v:'male',l:'Male'},{v:'female',l:'Female'},{v:'other',l:'Other'}].map(o=>({value:o.v,label:o.l}))} required />
            </div>
            <div className="grid grid-cols-2 gap-6">
              <FormField label="Phone" value={formData.phone} onChange={v => updateFormData('phone', v)} placeholder="(555) 000-0000" required error={errors.phone} />
              <FormField label="Email" type="email" value={formData.email} onChange={v => updateFormData('email', v)} placeholder="name@example.com" required error={errors.email} />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
          ...
          <div className="md:col-span-8 bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm space-y-6">
            <FormField label="Street Address" value={formData.address.street} onChange={v => updateFormData('address.street', v)} required error={errors['address.street']} />
            <div className="grid grid-cols-3 gap-4">
              <FormField label="City" value={formData.address.city} onChange={v => updateFormData('address.city', v)} required error={errors['address.city']} />
              <FormField label="State" value={formData.address.state} onChange={v => updateFormData('address.state', v)} placeholder="NY" required error={errors['address.state']} />
              <FormField label="ZIP" value={formData.address.zipCode} onChange={v => updateFormData('address.zipCode', v)} placeholder="10001" required error={errors['address.zipCode']} />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
          ...
          <div className="md:col-span-8 bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm space-y-6">
            <div className="grid grid-cols-2 gap-6">
              <FormField label="Contact Name" value={formData.emergencyContact.name} onChange={v => updateFormData('emergencyContact.name', v)} required error={errors['emergencyContact.name']} />
              <FormField label="Relationship" value={formData.emergencyContact.relationship} onChange={v => updateFormData('emergencyContact.relationship', v)} placeholder="Parent, Spouse..." required error={errors['emergencyContact.relationship']} />
            </div>
            <FormField label="Emergency Phone" value={formData.emergencyContact.phone} onChange={v => updateFormData('emergencyContact.phone', v)} required error={errors['emergencyContact.phone']} />
          </div>
        </div>
